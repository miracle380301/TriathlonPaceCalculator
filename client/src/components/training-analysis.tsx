...
import { Waves, Bike, Footprints, TrendingUp, Target, Calendar } from "lucide-react";
...
export default function TrainingAnalysis({ comparison, trainingPlan }: TrainingAnalysisProps) {
  const disciplines = [
    { 
      key: 'swim', 
      name: '수영', 
      ...
      <Card className="p-6">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          {disciplines.map((discipline) => {
            const data = comparison[discipline.key as keyof typeof comparison] as any;
            const isSlower = data.status.includes('느림');
            
            return (
              <div key={discipline.key} className={`${discipline.bgColor} rounded-xl p-5`}>
                <div className="flex items-center gap-3 mb-3">
                  <div className={`${discipline.color.replace('text-', 'bg-').replace('-500', '-500')} p-2 rounded-lg`}>
                    <discipline.icon className="text-white" size={16} />
                  </div>
                  <h3 className={`font-semibold ${discipline.color.replace('-500', '-700')}`}>
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm">
                    <Badge variant={isSlower ? "destructive" : "default"} className="text-xs">
                      {data.difference}
                    </Badge>
                  </div>
                </div>
        ...
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-semibold text-yellow-800">가장 개선이 필요한 구간</span>
          </div>
      ...
      <Card className="p-6">
        ...
        <div className="space-y-6">
          {trainingPlan.map((plan, index) => {
            const discipline = disciplines.find(d => d.key === plan.discipline);
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant={plan.priority === 1 ? "destructive" : "secondary"} className="text-xs">
                        우선순위 {plan.priority}
                      </Badge>
                    </div>
                    <p className="text-sm text-gray-600 mt-1">{plan.improvement_needed}</p>
                  </div>
                </div>
                  ...
                  {Object.entries(dayNames).map(([day, dayKor]) => {
                    ...
                    return (
                      <div key={day} className="text-center">
                        <div className="text-xs font-medium text-gray-500 mb-1">{dayKor}</div>
        ...
        <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-xl">
          ...
          <ul className="text-sm text-blue-700 space-y-1">
            ...
            <li>• 매주 훈련 기록을 Strava에 업로드하여 진행사항을 확인하세요</li>
          </ul>
        </div>
      </Card>
    </div>

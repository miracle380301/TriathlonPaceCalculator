...
export class StravaService {
  ...
  async syncUserActivities(userId: string): Promise<void> {
    ...
    const activities = await this.getActivities(user.stravaAccessToken, 1, 100);
    
    // Filter for triathlon activities (Swim, Ride, Run) from last 4 weeks
    const fourWeeksAgo = new Date();
    fourWeeksAgo.setDate(fourWeeksAgo.getDate() - 28);
    
    const triathlonActivities = activities
      ...
      .map(activity => ({
        ...
        averageSpeed: activity.average_speed ? Math.round(activity.average_speed * 1000) : null,
        activityData: activity,
      }));

    await storage.saveStravaActivities(triathlonActivities);
  }
}
